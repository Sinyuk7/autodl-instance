...
稳定背景：
- /root 是系统盘。正常关机不会清空，但重置系统或更换镜像会清空；实例释放后数据也会丢失。源码和可重建环境放这里。
- /root/autodl-tmp 是数据盘的预期挂载点。目录存在不代表数据盘真的已挂载，必须用 findmnt、mountpoint、df 等验证。模型、ComfyUI user/output 和大型缓存应放这里。
- /root/autodl-fs 是较慢的共享文件存储，适合压缩备份，不适合活跃模型和 ComfyUI 工作负载。
...
2. 存储：用 findmnt -T、mountpoint、df -hT、df -i 验证 /root、/root/autodl-tmp、/root/autodl-fs 的真实挂载、容量和 inode。只对关键目录做浅层大小统计，避免全盘扫描。
...
10. 安全与恢复：检查 secrets/config 权限、关键数据是否只有单份、是否已有备份位置；不得展示秘密内容。
...
建议保留的全局规则是：磁盘职责、先读后写、秘密保护、进程归属检查、非全局代理、真实挂载验证，以及“主机实测优先于文档默认值”。项目架构和测试命令继续由仓库内的 `AGENTS.md` 管理。
